    ...
    if accepted=="Accepted":
        ...
        if firstname and lastname:
            ...
            if not os.path.exists(filepath):
                ...
                heading = ["ID", "First Name", "Last Name", "Phone Number", "Address","Term Period", "Class Type", "Level"]
                sheet.append(heading)
                workbook.save(filepath)
            workbook = openpyxl.load_workbook(filepath)
            sheet = workbook.active
            sheet.append([student_id, firstname, lastname, phone_number, student_address, term_period, class_type, class_number])
            workbook.save(filepath)
